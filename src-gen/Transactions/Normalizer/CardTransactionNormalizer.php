...
class CardTransactionNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('cardId')) {
            $dataArray['card_id'] = $data->getCardId();
        }
        $dataArray['description'] = $data->getDescription();
        $dataArray['amount'] = $this->normalizer->normalize($data->getAmount(), 'json', $context);
        $dataArray['initiated_at_date'] = $data->getInitiatedAtDate()->format('Y-m-d');
        $dataArray['posted_at_date'] = $data->getPostedAtDate()->format('Y-m-d');
        if ($data->isInitialized('type') && null !== $data->getType()) {
            $dataArray['type'] = $data->getType();
        }
        if ($data->isInitialized('merchant') && null !== $data->getMerchant()) {
            $dataArray['merchant'] = $this->normalizer->normalize($data->getMerchant(), 'json', $context);
        }
        if ($data->isInitialized('cardMetadata')) {
            $values = [];
            foreach ($data->getCardMetadata() as $key => $value) {
                $values[$key] = $value;
            }
            $dataArray['card_metadata'] = $values;
        }
        if ($data->isInitialized('expenseId')) {
