...
class CreateTransferRequestNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \NxSys\Library\Clients\Brex\API\Payments\Model\CreateTransferRequest::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \NxSys\Library\Clients\Brex\API\Payments\Model\CreateTransferRequest::class;
    }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        $values_1 = [];
        foreach ($data->getOriginatingAccount() as $key_1 => $value_1) {
            $values_1[$key_1] = $value_1;
        }
        $dataArray['originating_account'] = $values_1;
        if ($data->isInitialized('approvalType') && null !== $data->getApprovalType()) {
            $dataArray['approval_type'] = $data->getApprovalType();
        }
        foreach ($data as $key_2 => $value_2) {
            if (preg_match('/.*/', (string) $key_2)) {
    ...
    {
        return [\NxSys\Library\Clients\Brex\API\Payments\Model\CreateTransferRequest::class => false];
    }
}