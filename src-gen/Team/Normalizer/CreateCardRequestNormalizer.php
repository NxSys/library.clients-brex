...
class CreateCardRequestNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \NxSys\Library\Clients\Brex\API\Team\Model\CreateCardRequest::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \NxSys\Library\Clients\Brex\API\Team\Model\CreateCardRequest::class;
    }
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('spend_controls', $data)) {
            $object->setSpendControls($this->denormalizer->denormalize($data['spend_controls'], \NxSys\Library\Clients\Brex\API\Team\Model\CreateCardRequestSpendControls::class, 'json', $context));
            unset($data['spend_controls']);
        }
        if (\array_key_exists('mailing_address', $data)) {
            $object->setMailingAddress($this->denormalizer->denormalize($data['mailing_address'], \NxSys\Library\Clients\Brex\API\Team\Model\CreateCardRequestMailingAddress::class, 'json', $context));
        ...
        if (\array_key_exists('card_attribute_preferences', $data)) {
            $object->setCardAttributePreferences($this->denormalizer->denormalize($data['card_attribute_preferences'], \NxSys\Library\Clients\Brex\API\Team\Model\CreateCardRequestCardAttributePreferences::class, 'json', $context));
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('cardAttributePreferences') && null !== $data->getCardAttributePreferences()) {
            $dataArray['card_attribute_preferences'] = $this->normalizer->normalize($data->getCardAttributePreferences(), 'json', $context);
        }
        foreach ($data as $key_2 => $value_2) {
            if (preg_match('/.*/', (string) $key_2)) {
    ...
    {
        return [\NxSys\Library\Clients\Brex\API\Team\Model\CreateCardRequest::class => false];
    }
}